 ...
 * @Author :
 * @Explain : the operation by redis
 * @contact:
...
public class RedisUtil {
     ...
     * @param value
     */
    public void setKeyKV(String key, String value) {
        redisTemplate.opsForValue().set(key, value);
    }
    ...
    public void delete(String... key) {
        ...
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                logger.info("delete key info{}:" + "delete one key is successfully.");
            } else {
                redisTemplate.delete(CollectionUtils.arrayToList(key));
                logger.info("delete key info{}:" + "delete some keys are successfully.");
